...
z1.chain <- readRDS("/Users/arhitchakrabarti/Library/CloudStorage/OneDrive-TexasA&MUniversity/GDP/JMLR_Revision/RealDataMultiChain/HDP/HDP_z_chain_l1_alpha_3_L_30.rds")
z2.chain <- readRDS("/Users/arhitchakrabarti/Library/CloudStorage/OneDrive-TexasA&MUniversity/GDP/JMLR_Revision/RealDataMultiChain/HDP/HDP_z_chain_l2_alpha_3_L_30.rds")
z3.chain <- readRDS("/Users/arhitchakrabarti/Library/CloudStorage/OneDrive-TexasA&MUniversity/GDP/JMLR_Revision/RealDataMultiChain/HDP/HDP_z_chain_l3_alpha_3_L_30.rds")
z4.chain <- readRDS("/Users/arhitchakrabarti/Library/CloudStorage/OneDrive-TexasA&MUniversity/GDP/JMLR_Revision/RealDataMultiChain/HDP/HDP_z_chain_l4_alpha_3_L_30.rds")
...
library(mcclust.ext)
posterior_samples <- matrix(0, nrow = length(z_samples), ncol = length(z_samples[[1]]))

for(i in 1:length(z_samples)){
  posterior_samples[i, ] = z_samples[[i]]
}
...
cluster.global_mcclust2 <- data.frame(x = c(X1[1, ],
                                            X2[1, ],
                                            X3[1, ],
                                            X4[1, ],
                                            X5[1, ],
                                      ...
                                      group = factor(c(rep("Population 1", length(X1[1,])),
                                                       rep("Population 2", length(X2[1,])),
                                                       rep("Population 3", length(X3[1,])),
                                                       rep("Population 4", length(X4[1,])),
                                                       rep("Population 5", length(X5[1,])),
                                                       rep("Population 6", length(X6[1,])),
                                                       rep("Population 7", length(X7[1,])),
                                                       rep("Population 8", length(X8[1,])))))


################################################################################
## POST PROCESSING
################################################################################
z1_samples.post.burn = cluster.global_mcclust2 %>% filter(group == "Population 1") %>% pull(cluster)
singleton1 = as.numeric(names(which(table(z1_samples.post.burn) <= 0.05 * n1)))
singleton.index1 = which(z1_samples.post.burn %in% singleton1)
if(length(singleton.index1) > 0){
  z1.estimated = z1_samples.post.burn[-singleton.index1]
}else{
  z1.estimated = z1_samples.post.burn
}

z2_samples.post.burn = cluster.global_mcclust2 %>% filter(group == "Population 2") %>% pull(cluster)
singleton2 = as.numeric(names(which(table(z2_samples.post.burn) <= 0.05 * n2)))
singleton.index2 = which(z2_samples.post.burn %in% singleton2)
if(length(singleton.index2) > 0){
  z2.estimated = z2_samples.post.burn[-singleton.index2]
}else{
  z2.estimated = z2_samples.post.burn
}

z3_samples.post.burn = cluster.global_mcclust2 %>% filter(group == "Population 3") %>% pull(cluster)
singleton3 = as.numeric(names(which(table(z3_samples.post.burn) <= 0.05 * n3)))
singleton.index3 = which(z3_samples.post.burn %in% singleton3)
if(length(singleton.index3) > 0){
  z3.estimated = z3_samples.post.burn[-singleton.index3]
}else{
  z3.estimated = z3_samples.post.burn
}


z4_samples.post.burn = cluster.global_mcclust2 %>% filter(group == "Population 4") %>% pull(cluster)
singleton4 = as.numeric(names(which(table(z4_samples.post.burn) <= 0.05 * n4)))
singleton.index4 = which(z4_samples.post.burn %in% singleton4)
if(length(singleton.index4) > 0){
  z4.estimated = z4_samples.post.burn[-singleton.index4]
}else{
  z4.estimated = z4_samples.post.burn
}

z5_samples.post.burn = cluster.global_mcclust2 %>% filter(group == "Population 5") %>% pull(cluster)
singleton5 = as.numeric(names(which(table(z5_samples.post.burn) <= 0.05 * n5)))
singleton.index5 = which(z5_samples.post.burn %in% singleton5)
if(length(singleton.index5) > 0){
  z5.estimated = z5_samples.post.burn[-singleton.index5]
}else{
  z5.estimated = z5_samples.post.burn
}


z6_samples.post.burn = cluster.global_mcclust2 %>% filter(group == "Population 6") %>% pull(cluster)
singleton6 = as.numeric(names(which(table(z6_samples.post.burn ) <= 0.05 * n6)))
singleton.index6 = which(z6_samples.post.burn  %in% singleton6)
if(length(singleton.index6) > 0){
  z6.estimated = z6_samples.post.burn [-singleton.index6]
}else{
  z6.estimated = z6_samples.post.burn 
}

z7_samples.post.burn = cluster.global_mcclust2 %>% filter(group == "Population 7") %>% pull(cluster)
singleton7 = as.numeric(names(which(table(z7_samples.post.burn) <= 0.05 * n7)))
singleton.index7 = which(z7_samples.post.burn %in% singleton7)
if(length(singleton.index7) > 0){
  z7.estimated = z7_samples.post.burn[-singleton.index7]
}else{
  z7.estimated =z7_samples.post.burn
}

z8_samples.post.burn = cluster.global_mcclust2 %>% filter(group == "Population 8") %>% pull(cluster)
singleton8 = as.numeric(names(which(table(z8_samples.post.burn) <= 0.05 * n8)))
singleton.index8 = which(z8_samples.post.burn %in% singleton8)
if(length(singleton.index8) > 0){
  z8.estimated = z8_samples.post.burn[-singleton.index8]
}else{
  z8.estimated = z8_samples.post.burn
}
