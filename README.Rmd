...
```
**Note**: The library *devtools* needs to be installed before installing the R package from GitHub. Here is a detailed example of a simulation study. The example may be used to understand the various functionalities of the R package.

## Simulation Example 

We first generate the data from the eight dependent groups. The dependency between the groups can be described by the underlying DAG as mentioned in the paper.
...
Once we have our data, we first run a vanilla DP for each group. We estimate of the stick breaking weights for each group from this vanilla DP. We will use this estimated stick-breaking weights as initial values in our proposed GDP sampler. This ensures fast mixing of the proposed MCMC sampler.
...
alpha8.start = rgamma(n = 1, shape = (alpha5.start + alpha5.start +
                                        alpha7.start))
...
for(i in 1:L.max){
  mu.start.list[[i]] = matrix(mvtnorm::rmvnorm(n = 1,
                                               mean = as.numeric(mu0), 
...
alpha0 = 2; nu = nrow(X1); W = diag(1, nrow = nu); mu0 = as.matrix(rep(0, nu), ncol = 1); lambda = 0.01 
...
gibbs <- GDPSamp::gibbs_MVNW_dp_c(init = init, data = X1, num_iterations = num_iterations, burn = burn, thin = 1, L.max = L.max, prior = prior)
...
beta1.start.long = c(beta1.start.small, rep(1e-6, abs(L.max - length(beta1.start.small))))
...
alpha0 = 2; nu = nrow(X2); W = diag(1, nrow = nu); mu0 = as.matrix(rep(0, nu), ncol = 1); lambda = 0.01 
# Prior hyper-parameters
prior = list(alpha0 = alpha0, nu = nu, W = W, prior_mean = mu0, prior_prec = lambda)
gibbs <- GDPSamp::gibbs_MVNW_dp_c(init = init, data = X2, num_iterations = num_iterations, burn = burn, thin = 1, L.max = L.max, prior = prior)
...
# So I add a very small quantity to the remaining co-ordinates and re-normlaize
beta2.start.long = c(beta2.start.small, rep(1e-6, abs(L.max - length(beta2.start.small))))
...
alpha0 = 2; nu = nrow(X3); W = diag(1, nrow = nu); mu0 = as.matrix(rep(0, nu), ncol = 1); lambda = 0.01 
# Prior hyper-parameters
prior = list(alpha0 = alpha0, nu = nu, W = W, prior_mean = mu0, prior_prec = lambda)
gibbs <- GDPSamp::gibbs_MVNW_dp_c(init = init, data = X3, num_iterations = num_iterations, burn = burn, thin = 1, L.max = L.max, prior = prior)
...
# So I add a very small quantity to the remaining co-ordinates and re-normlaize
beta3.start.long = c(beta3.start.small, rep(1e-6, abs(L.max - length(beta3.start.small))))
...
alpha0 = 2; nu = nrow(X4); W = diag(1, nrow = nu); mu0 = as.matrix(rep(0, nu), ncol = 1); lambda = 0.01 
# Prior hyper-parameters
prior = list(alpha0 = alpha0, nu = nu, W = W, prior_mean = mu0, prior_prec = lambda)
gibbs <- GDPSamp::gibbs_MVNW_dp_c(init = init, data = X4, num_iterations = num_iterations, burn = burn, thin = 1, L.max = L.max, prior = prior)
...
# So I add a very small quantity to the remaining co-ordinates and re-normlaize
beta4.start.long = c(beta4.start.small, rep(1e-6, abs(L.max - length(beta4.start.small))))
...
alpha0 = 2; nu = nrow(X5); W = diag(1, nrow = nu); mu0 = as.matrix(rep(0, nu), ncol = 1); lambda = 0.01 
# Prior hyper-parameters
prior = list(alpha0 = alpha0, nu = nu, W = W, prior_mean = mu0, prior_prec = lambda)
gibbs <- GDPSamp::gibbs_MVNW_dp_c(init = init, data = X5, num_iterations = num_iterations, burn = burn, thin = 1, L.max = L.max, prior = prior)
...
# So I add a very small quantity to the remaining co-ordinates and re-normlaize
beta5.start.long = c(beta5.start.small, rep(1e-6, abs(L.max - length(beta5.start.small))))
...
alpha0 = 2; nu = nrow(X6); W = diag(1, nrow = nu); mu0 = as.matrix(rep(0, nu), ncol = 1); lambda = 0.01 
# Prior hyper-parameters
prior = list(alpha0 = alpha0, nu = nu, W = W, prior_mean = mu0, prior_prec = lambda)
gibbs <- GDPSamp::gibbs_MVNW_dp_c(init = init, data = X6, num_iterations = num_iterations, burn = burn, thin = 1, L.max = L.max, prior = prior)
...
# So I add a very small quantity to the remaining co-ordinates and re-normlaize
beta6.start.long = c(beta6.start.small, rep(1e-6, abs(L.max - length(beta6.start.small))))
...
alpha0 = 2; nu = nrow(X7); W = diag(1, nrow = nu); mu0 = as.matrix(rep(0, nu), ncol = 1); lambda = 0.01 
# Prior hyper-parameters
prior = list(alpha0 = alpha0, nu = nu, W = W, prior_mean = mu0, prior_prec = lambda)
gibbs <- GDPSamp::gibbs_MVNW_dp_c(init = init, data = X7, num_iterations = num_iterations, burn = burn, thin = 1, L.max = L.max, prior = prior)
...
# So I add a very small quantity to the remaining co-ordinates and re-normlaize
beta7.start.long = c(beta7.start.small, rep(1e-6, abs(L.max - length(beta7.start.small))))
...
alpha0 = 2; nu = nrow(X8); W = diag(1, nrow = nu); mu0 = as.matrix(rep(0, nu), ncol = 1); lambda = 0.01 
# Prior hyper-parameters
prior = list(alpha0 = alpha0, nu = nu, W = W, prior_mean = mu0, prior_prec = lambda)
gibbs <- GDPSamp::gibbs_MVNW_dp_c(init = init, data = X8, num_iterations = num_iterations, burn = burn, thin = 1, L.max = L.max, prior = prior)
...
# So I add a very small quantity to the remaining co-ordinates and re-normlaize
beta8.start.long = c(beta8.start.small, rep(1e-6, abs(L.max - length(beta8.start.small))))
...
Once we have all the initilializations, we are ready to run our proposed GDP Sampler. We consider our sampler here, wherein we consider 25,000 iterations of our sampler and 10,000 burn-in and thinning by a factor of 10.
...
beta1_samples[[1]] <- rep(1/L.max, L.max); beta2_samples[[1]] <- rep(1/L.max, L.max); beta3_samples[[1]] <- rep(1/L.max, L.max); beta4_samples[[1]] <- rep(1/L.max, L.max); beta5_samples[[1]] <- rep(1/L.max, L.max); beta6_samples[[1]] <- rep(1/L.max, L.max); beta7_samples[[1]] <- rep(1/L.max, L.max); beta8_samples[[1]] <- rep(1/L.max, L.max);
nu1_samples[[1]] <- rep(1/L.max, L.max); nu2_samples[[1]] <- rep(1/L.max, L.max); nu3_samples[[1]] <- rep(1/L.max, L.max); nu4_samples[[1]] <- rep(1/L.max, L.max)
...
# Rand index
rand_gdp_1 <- list(); rand_gdp_2 <- list(); rand_gdp_3 <- list(); rand_gdp_4 <- list(); rand_gdp_5 <- list(); rand_gdp_6 <- list(); rand_gdp_7 <- list(); rand_gdp_8 <- list()
...
for(b in 2:gibbs_iterations){
  ...
  beta1_samples[[b]] = beta1_samples[[b]] + 1e-6 
  ...
  beta2_samples[[b]] = as.numeric(extraDistr::rdirichlet(n = 1, alpha = n2_samples[[b]] + new_concentration))
  ...
  beta2_samples[[b]] = beta2_samples[[b]] + 1e-6 
  ...
  # Now we only need to consider components 1, 2,...,L.obs 
  beta3_samples[[b]] = as.numeric(extraDistr::rdirichlet(n = 1, alpha = n3_samples[[b]] + new_concentration))
  # To avoid numerical issues I add a small quantity to each coordinate and re-normalize
  beta3_samples[[b]][is.na(beta3_samples[[b]] )] <- 0
  
  beta3_samples[[b]] = beta3_samples[[b]] + 1e-6 
  ...
  # Now we only need to consider components 1, 2,...,L.obs 
  beta4_samples[[b]] = as.numeric(extraDistr::rdirichlet(n = 1, alpha = n4_samples[[b]] + new_concentration))
  # To avoid numerical issues I add a small quantity to each coordinate and re-normalize
  beta4_samples[[b]][is.na(beta4_samples[[b]] )] <- 0
  
  beta4_samples[[b]] = beta4_samples[[b]] + 1e-6 
  ...
  init = nu1.start
  nu1 = GDPSamp::RunMH_my(Target = GDPSamp::Target_nu1, 
                          init = init, B = 1,
                          h = rep(2, L.obs), 
                          pars = list(alpha2 = alpha2_samples[[b-1]],
                                      alpha4 = alpha4_samples[[b-1]],
  ...
  nu1_samples[[b]] = nu1_samples[[b]] + 1e-6
  ...
  # Now we only need to consider components 1, 2,...,L.obs 
  beta5_samples[[b]] = as.numeric(extraDistr::rdirichlet(n = 1, alpha = n5_samples[[b]] + new_concentration))
  # To avoid numerical issues I add a small quantity to each coordinate and re-normalize
  beta5_samples[[b]][is.na(beta5_samples[[b]] )] <- 0
  beta5_samples[[b]] = beta5_samples[[b]] + 1e-6 
  ...
  init = nu2.start
  nu2 = GDPSamp::RunMH_my(Target = GDPSamp::Target_nu2, 
                          init = init, B = 1,
                          h = rep(2, L.obs), 
                          pars = list(alpha2 = alpha2_samples[[b-1]],
                                      alpha3 = alpha3_samples[[b-1]],
  ...
  nu2_samples[[b]] = nu2_samples[[b]] + 1e-6
  ...
  # Now we only need to consider components 1, 2,...,L.obs 
  beta6_samples[[b]] = as.numeric(extraDistr::rdirichlet(n = 1, alpha = n6_samples[[b]] + new_concentration))
  # To avoid numerical issues I add a small quantity to each coordinate and re-normalize
  beta6_samples[[b]][is.na(beta6_samples[[b]] )] <- 0
  beta6_samples[[b]] = beta6_samples[[b]] + 1e-6 
  ...
  init = nu3.start
  nu3 = GDPSamp::RunMH_my(Target = GDPSamp::Target_nu3, 
                          init = nu3.start, B = 1,
                          h = rep(2, L.obs), 
  ...
  nu3_samples[[b]] = nu3_samples[[b]] + 1e-6
  ...
  # Now we only need to consider components 1, 2,...,L.obs 
  beta7_samples[[b]] = as.numeric(extraDistr::rdirichlet(n = 1, alpha = n7_samples[[b]] + new_concentration))
  # To avoid numerical issues I add a small quantity to each coordinate and re-normalize
  beta7_samples[[b]][is.na(beta7_samples[[b]] )] <- 0
  beta7_samples[[b]] = beta7_samples[[b]] + 1e-6 
  ...
  init = eta.start
  eta = GDPSamp::RunMH_my(Target = GDPSamp::Target_eta, 
                          init = init, B = 1,
                          h = rep(2, L.obs), 
                          pars = list(alpha2 = alpha2_samples[[b-1]],
                                      alpha3 = alpha3_samples[[b-1]],
  ...
  eta_samples[[b]] = eta_samples[[b]] + 1e-6
  ...
  init = nu4.start
  nu4 = GDPSamp::RunMH_my(Target = GDPSamp::Target_nu4, 
                          init = init, B = 1,
                          h = rep(2, L.obs), 
  ...
  nu4_samples[[b]] = nu4_samples[[b]] + 1e-6
  ...
  # Now we only need to consider components 1, 2,...,L.obs 
  beta8_samples[[b]] = as.numeric(extraDistr::rdirichlet(n = 1, alpha = n8_samples[[b]] + new_concentration))
  # To avoid numerical issues I add a small quantity to each coordinate and re-normalize
  beta8_samples[[b]][is.na(beta8_samples[[b]] )] <- 0
  beta8_samples[[b]] = beta8_samples[[b]] + 1e-6 
  ...
  alpha1_samples[[b]] = GDPSamp::sample_alpha1(ycurrent = alpha1_samples[[b-1]], 
  ...
  alpha2_samples[[b]] = GDPSamp::sample_alpha2(ycurrent = alpha2_samples[[b-1]], 
  ...
  alpha3_samples[[b]] = GDPSamp::sample_alpha3(ycurrent = alpha3_samples[[b-1]], 
                                     pars = list(alpha1 = alpha1_samples[[b]], 
  ...
  alpha4_samples[[b]] = GDPSamp::sample_alpha4(ycurrent = alpha4_samples[[b-1]], 
                                     pars = list(alpha1 = alpha1_samples[[b]], 
                                                 alpha2 = alpha2_samples[[b]], 
  ...
  # beta5, nu1, nu4, eta
  alpha5_samples[[b]] = GDPSamp::sample_alpha5(ycurrent = alpha5_samples[[b-1]], 
  ...
  # beta6, nu2, nu4, eta
  alpha6_samples[[b]] = GDPSamp::sample_alpha6(ycurrent = alpha6_samples[[b-1]], 
                                     pars = list(alpha2 = alpha2_samples[[b]], 
                                                 alpha3 = alpha3_samples[[b]], 
  ...
  # beta7, nu3, nu4, eta
  alpha7_samples[[b]] = GDPSamp::sample_alpha7(ycurrent = alpha7_samples[[b-1]], 
  ...
  # beta8, nu4
  alpha8_samples[[b]] = GDPSamp::sample_alpha8(ycurrent = alpha8_samples[[b-1]], 
...
M1.Frobenius <- sapply(M1, function(x, av) sum((x - av)^2),
                       av = M1.mean)
...
# Calculate the Frobenius norm of the differences
M2.Frobenius <- sapply(M2, function(x, av) sum((x - av)^2),
                       av = M2.mean)
# Find out the minimums
...
# Calculate the Frobenius norm of the differences
M3.Frobenius <- sapply(M3, function(x, av) sum((x - av)^2),
                       av = M3.mean)
# Find out the minimums
...
# Calculate the Frobenius norm of the differences
M4.Frobenius <- sapply(M4, function(x, av) sum((x - av)^2),
                       av = M4.mean)
# Find out the minimums
...
# Calculate the Frobenius norm of the differences
M5.Frobenius <- sapply(M5, function(x, av) sum((x - av)^2),
                       av = M5.mean)
# Find out the minimums
...
# Calculate the Frobenius norm of the differences
M6.Frobenius <- sapply(M6, function(x, av) sum((x - av)^2),
                       av = M6.mean)
# Find out the minimums
...
# Calculate the Frobenius norm of the differences
M7.Frobenius <- sapply(M7, function(x, av) sum((x - av)^2),
                       av = M7.mean)
# Find out the minimums
...
# Calculate the Frobenius norm of the differences
M8.Frobenius <- sapply(M8, function(x, av) sum((x - av)^2),
                       av = M8.mean)
# Find out the minimums
...
singleton1 = as.numeric(names(which(table(z1_samples.post.burn[[k1.min]]) <= 0.02 * n1)))
...
singleton2 = as.numeric(names(which(table(z2_samples.post.burn[[k2.min]]) <= 0.02 * n2)))
...
singleton3 = as.numeric(names(which(table(z3_samples.post.burn[[k3.min]]) <= 0.02 * n3)))
...
singleton4 = as.numeric(names(which(table(z4_samples.post.burn[[k4.min]]) <= 0.02 * n4)))
...
singleton5 = as.numeric(names(which(table(z5_samples.post.burn[[k5.min]]) <= 0.02 * n5)))
...
singleton6 = as.numeric(names(which(table(z6_samples.post.burn[[k6.min]]) <= 0.02 * n6)))
...
singleton7 = as.numeric(names(which(table(z7_samples.post.burn[[k7.min]]) <= 0.02 * n7)))
...
singleton8 = as.numeric(names(which(table(z8_samples.post.burn[[k8.min]]) <= 0.02 * n8)))
...
Plot of the log-likelihood and clustering plots can be obtained as below:
